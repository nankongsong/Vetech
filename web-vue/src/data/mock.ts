...
/** 构建业务类型树 */
export function buildBusinessTypeTree(): BusinessTypeNode[] {
  const map = new Map<string, BusinessTypeNode>()
  businessTypes.forEach(n => map.set(n.businessTypeId, { ...n, children: [] }))
  const roots: BusinessTypeNode[] = []
  map.forEach(n => {
...
export function cityMealStandard(cityNo: string): number {
  const city = cities.find(c => c.cityNo === cityNo)
  if (!city) return 0
  return city.cityType === '1' ? 100 : city.cityType === '2' ? 80 : 50
}

export const cityTrafficStandard = (): number => 40
export const cityCommStandard = (): number => 40
