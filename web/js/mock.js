...
/* 工具：构建业务类型树 */
window.MOCK.buildBusinessTypeTree = function () {
  const list = window.MOCK.businessTypes;
  const map = new Map();
...
/* 工具：将城市类型转成补助标准 */
window.MOCK.cityMealStandard = function (cityNo) {
  const city = window.MOCK.cities.find(c => c.cityNo === cityNo);
  if (!city) return 0;
  return city.cityType === '1' ? 100 : (city.cityType === '2' ? 80 : 50);
};
window.MOCK.cityTrafficStandard = function () { return 40; };
window.MOCK.cityCommStandard = function () { return 40; };
