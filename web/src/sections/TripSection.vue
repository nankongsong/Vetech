...
async function onSave(data: Omit<Trip, 'id'>) {
  ...
  if (conflict) {
    const emp = store.employees.find(e => e.reimbursementId === data.reimbursementId)
    await confirm.alert(`出行人 ${emp ? emp.reimbursementName : ''} 在 ${conflict.startDate} 至 ${conflict.endDate} 已存在行程，不可重复`)
    return
  }
